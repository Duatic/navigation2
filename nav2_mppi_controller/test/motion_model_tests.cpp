  ...
  const float vx_max, const float vx_min, const float vy_max, const float wz_max)
{
  ...
  model.constrainControlSequence(control_sequence, geometry_msgs::msg::Twist(), 0.1f, false);
}
...
TEST(MotionModelTests, DiffDriveTest)
{
  ...
  model->constrainControlSequence(
    control_sequence, geometry_msgs::msg::Twist(), 0.1f, false);
...
TEST(MotionModelTests, AckermannTurningRadiusTest)
{
  ...
  // Braking while turning tightens the turning radius bound as vx falls. That must not pull wz
  // down faster than the angular deceleration limit allows.
  models::ControlSequence control_sequence;
  control_sequence.reset(2);  // populates with zeros
  ...
  speed.linear.x = 1.0;
  speed.angular.z = 4.0;
  model->constrainControlSequence(control_sequence, speed, 0.1f, false);

  EXPECT_NEAR(control_sequence.wz(0), 4.0f - 0.1f * constraints.az_max, 1e-5);
  EXPECT_LE(std::fabs(control_sequence.wz(0)), std::fabs(control_sequence.vx(0)) / r + 1e-5f);

  // A turning radius of zero is not validated anywhere, and neither is calling this before
  // initialize(). At a standstill that would divide zero by zero, so check the horizon stays
  // finite: rotation is still bounded by |vx| / r, which is zero when the vehicle is stopped.
  ...
  EXPECT_TRUE((control_sequence.wz == 0.0f).all());

  // While moving, a zero radius leaves wz bounded by its own limit alone
  control_sequence.vx.setConstant(1.0f);
  control_sequence.wz.setConstant(50.0f);
  constrainWithFreeAcceleration(*zero_radius, control_sequence, 1.0f, -1.0f, 0.0f, 2.0f);

  EXPECT_TRUE(control_sequence.wz.isFinite().all());
  EXPECT_NEAR(control_sequence.wz(1), 2.0f, 1e-6);
